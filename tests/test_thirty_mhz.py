...
def test_sensor_type_exists():
    ...
    name = str(random())
    id = str(random())
    c = tmz.sensor_type.create(
        id=id,
        name=name,
        data=[{"testdata": 1, "testdata2": 2}, {"testdata": 3, "testdata3": 4}],
    )
    print(c)
...
def test_create_i_temp_import_check():
    ...
    id = str(random())
    data = [{"data": [{"temp": 13}]}]
    import_check = tmz.import_check.create(
        id=id, name="test_i_temp", sensor_type={"typeId": "i_temp"}
    )
            ...
            [
                {
                    ...
                    "name": f"testdata {id}",
                    "data": [
                        {"result_value": random_result_value, "file": f},
                    ],
                }
            ]
        )
